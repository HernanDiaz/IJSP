...
namespace IJSP {
	...
	//=============================================================================
	//-----  Copy constructor  ----------------------------------------------------
	NB_ParallelN1_ESDavgIJSP::NB_ParallelN1_ESDavgIJSP(
		const NB_ParallelN1_ESDavgIJSP & source)
	...
	//-----  Setup method  --------------------------------------------------------
	void NB_ParallelN1_ESDavgIJSP::setup(FuzzyFW::ParameterDB *parameters) {
	...
	void NB_ParallelN1_ESDavgIJSP::setInitialSolution(FuzzyFW::Solution *solution,
		...
		if (this->schedule == NULL) {
			...
			errorMsg += " of neighbourhood. Only Interval JSP Schedules are allowed.";
			throw new IJSPException("Neighbourhood", errorMsg);
		}
		...
		if (problem == NULL) {
			std::string errorMsg = "This neighbourhood can be applied only";
	...
	//-----  Find enighbours  -----------------------------------------------------
	unsigned int NB_ParallelN1_ESDavgIJSP::findNewNeighbours(
	...
	FuzzyFW::Fitness * NB_ParallelN1_ESDavgIJSP::evaluateNeighbour(
	...
	void NB_ParallelN1_ESDavgIJSP::acceptNeighbour(const unsigned int idx,
		const FuzzyFW::SharedVars *svars) {
		char tailChange;
		int taskJob;
		int z, msz, jsz;
	...
	//-----  Get the estimation  --------------------------------------------------
	FuzzyFW::Fitness * NB_ParallelN1_ESDavgIJSP::getEstimation(
		...
		if (!this->neighbours[idx]->isEstimated()) {
			if (this->estimator == NB_ParallelN1_ESDavgIJSP::ESTIM_HEADTAILS)
	...
	//-----  Discard a neighbour  --------------------------------------------------
	void NB_ParallelN1_ESDavgIJSP::discardNeighbour(const unsigned int idx) {
		if (idx < 0 || idx >= this->numNeighbours || this->neighbours[idx] == NULL) {
			std::string errorMsg = "Trying to acces a non-existing neighbour";
			throw new IJSPException("Neighbourhood", errorMsg);
		}
	...
	void NB_ParallelN1_ESDavgIJSP::sortByEstimation(
		const FuzzyFW::SharedVars *svars) {
	...
	//-----  Quick sort  ----------------------------------------------------------
	void NB_ParallelN1_ESDavgIJSP::quickSort(const int left, const int right,
		const FuzzyFW::SharedVars *svars) {
	...
	FuzzyFW::Neighbour* NB_ParallelN1_ESDavgIJSP::getNeighbour(const unsigned int idx) {
		if (idx < 0 || idx >= this->numNeighbours || this->neighbours[idx] == NULL) {
			std::string errorMsg = "Trying to acces a non-existing neighbour";
			throw new IJSPException("Neighbourhood", errorMsg);
		}
	...
	//-----  Estimate the quality of a neighbour through heads and tails  ---------
	void NB_ParallelN1_ESDavgIJSP::estimateHeadsTails(const ProblemIJSP *problem,
		const unsigned int idx) {
		if (idx < 0 || idx >= this->numNeighbours) {
			std::string errorMsg = "Trying to acces a non-existing neighbour";
			throw new IJSPException("Neighbourhood", errorMsg);
	...
	//=============================================================================
	//
	//	Class NB_ParallelN1_ESDminIJSP
	//
	//=============================================================================
	//=============================================================================
	//		METHODS
	//=============================================================================
	//-----  Find neighbours  -----------------------------------------------------
	unsigned int NB_ParallelN1_ESDminIJSP::findNewNeighbours(
		const FuzzyFW::SharedVars *svars) {
	...
	//-----  Fully evaluate the neighbour  ----------------------------------------
	FuzzyFW::Fitness * NB_ParallelN1_ESDminIJSP::evaluateNeighbour(
		const unsigned int idx, const FuzzyFW::SharedVars *svars,
		const bool improvement) {
	...
	//-----  Estimate the quality of a neighbour through heads and tails  ---------
	void NB_ParallelN1_ESDminIJSP::estimateHeadsTails(const ProblemIJSP *problem,
		const unsigned int idx) {
		if (idx < 0 || idx >= this->numNeighbours) {
			std::string errorMsg = "Trying to acces a non-existing neighbour";
			throw new IJSPException("Neighbourhood", errorMsg);
