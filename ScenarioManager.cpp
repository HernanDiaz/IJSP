...
namespace PostExecution {
	...
	void ScenarioManager::loadScenarios(const IJSP::ProblemIJSP *problem, std::string outputPrefix, const int numScenarios) {
		// Open output file for scenarios
	...
	int ScenarioManager::getRandomDuration(const IJSP::TaskIJSP* task) {
		return this->rnd.getInteger(task->p.a, task->p.b);
	}

	int ScenarioManager::getRandomDuration(int a, int b) {
		return this->rnd.getInteger(a,b);
	}

	inline bool ScenarioManager::fileExists(const std::string& name) {
		struct stat buffer;
		return (stat(name.c_str(), &buffer) == 0);
	}


	const unsigned int ScenarioManager::getDuration(const unsigned int numTask, const unsigned int numSimulation) {
		return this->crispTimes[numSimulation][numTask];
	}

	const unsigned int ScenarioManager::getDueDate(const unsigned int numJob, const unsigned int numSimulation) {
