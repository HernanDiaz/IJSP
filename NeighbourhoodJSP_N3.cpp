/*
 * NeighbourhoodJSP_N3.cpp
 *
 *  Created on: Oct 12, 2019
 *      Author: hdiaz
 */

#include "NeighbourhoodJSP_N3.h"

namespace JSP {

//-----  Find neighbours  -----------------------------------------------------
unsigned int NB_ParallelN3_MakespanJSP::findNewNeighbours(
...
FuzzyFW::Fitness * NB_ParallelN3_MakespanJSP::evaluateNeighbour(
...
void NB_ParallelN3_MakespanJSP::acceptNeighbour(const unsigned int idx,
	const FuzzyFW::SharedVars *svars) {
	int job;
	int z, msz, jsz;
...
//-----  Add a neighbour  -----------------------------------------------------
void NB_ParallelN3_MakespanJSP::addNeighbour(const unsigned int x,
	...
	if (this->numNeighbours < this->neighbours.size()
		...
		&& this->neighbours[this->numNeighbours] == nullptr) {
		this->neighbours[this->numNeighbours] = std::make_unique<NeighbourJSP_Arc>(x, y, z, tipo);
	}
	else {
		this->neighbours.push_back(std::make_unique<NeighbourJSP_Arc>(x, y, z, tipo));
	}
	this->numNeighbours++;
}
...
//-----  Discard a neighbour  -------------------------------------------------
void NB_ParallelN3_MakespanJSP::discardNeighbour(const unsigned int idx) {
	if (idx < 0 || idx >= this->numNeighbours || this->neighbours[idx] == nullptr) {
...
void NB_ParallelN3_MakespanJSP::estimateHeadsTails(const unsigned int idx) {
