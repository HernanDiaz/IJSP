...
\multirow{4}{*}{GA}
 & A0  & 100\% & ---   & ---      & 0.94 & --- \\
 & V2  & 38\%  & 20/32 & 0.38     & 0.86 & $-0.08$ $[-0.14,-0.02]$ \\
 & MOR & 2\%   & 1/48  & $<0.001$ & 0.99 & $+0.05$ $[+0.03,+0.08]$ \\
 & MIX & 46\%  & 24/28 & 0.68     & 0.83 & $-0.12$ $[-0.18,-0.06]$ \\
\midrule
\multirow{4}{*}{ABCE3}
 & A0  & 100\% & ---   & ---      & 0.67 & --- \\
 & V2  & 90\%  & 54/4  & $<0.001$ & 0.29 & $-0.38$ $[-0.45,-0.30]$ \\
 & MOR & 38\%  & 19/37 & 0.045    & 0.81 & $+0.14$ $[+0.06,+0.23]$ \\
 & MIX & 95\%  & 57/3  & $<0.001$ & \textbf{0.20} & $\mathbf{-0.47}$ $[-0.54,-0.40]$ \\
\midrule
\multirow{4}{*}{fEABCLS}
 & A0  & 100\% & ---   & ---      & 0.90 & --- \\
 & V2  & 56\%  & 32/19 & 0.32     & 0.75 & $-0.15$ $[-0.22,-0.08]$ \\
 & MOR & 5\%   & 1/43  & $<0.001$ & 0.99 & $+0.09$ $[+0.06,+0.11]$ \\
 & MIX & 48\%  & 29/20 & 0.32     & 0.76 & $-0.14$ $[-0.22,-0.07]$ \\
\midrule
\multirow{4}{*}{TSN2}
 & A0  & 100\% & ---   & ---      & 0.86 & --- \\
 & V2  & 49\%  & 24/34 & 0.71     & 0.83 & $-0.03$ $[-0.09,+0.03]$ \\
 & MOR & 23\%  & 14/43 & $<0.001$ & 0.88 & $+0.02$ $[-0.04,+0.08]$ \\
 & MIX & 66\%  & 39/19 & 0.048    & 0.72 & $-0.14$ $[-0.21,-0.07]$ \\
\bottomrule
\end{tabular}
\end{table}
...
\emph{final} quality earlier on 57 of the 60 instances that are not ties,
cutting restricted mean time from $0.67$ to $0.20$ of the budget; V2 and
V2H do nearly as well, and all six seeded arms are significant on both
contrasts --- five faster, MOR slower. These are unambiguous ``same
quality, sooner'' results, and they agree with the converged-quality
finding for that solver.

Only TSN2 joins it, and marginally: its mixed pool is earlier on 39
instances and later on 19 ($p=0.048$), the only other arm anywhere to clear
the sign test in the direction of acceleration. On the GA the mixed pool is
earlier on 24 and later on 28 --- a coin flip --- and on fEABCLS no arm
clears the test either.

The two contrasts disagree on the GA and fEABCLS, and on the same three
arms in both: $\Delta$RMTT is negative and significant for V2H, V2 and MIX,
whose sign tests are not. That pattern has a definite reading. A mean is
pulled down by a minority of instances on which the arm arrives much
earlier, while the majority do not improve at all; the sign test is what
separates a general acceleration from one concentrated in a subset. Outside
ABCE3 and the mixed pool on TSN2, seeding buys the second, and the
distinction matters to a practitioner deciding whether to seed a given run.

Eight arm--solver pairs are significantly \emph{slower} to the target than
their control, and the weakest two generators account for all of them. MOR
is slower on all four solvers: it reaches the target on 1 instance of 61 on
the GA and on 3 on fEABCLS. GT is slower on three, being faster only on
ABCE3. Both sit at the bottom of the quality ladder of
...
either time-to-target metric. Metric A separates nothing once curves are
made monotone, and under metric B no fEABCLS arm clears the paired sign
test: its mixed pool is earlier on 29 instances and later on 20
(Section~\ref{sec:res-ttt}). What remains is the budget-fraction table,
where fEABCLS improves from $14.84\%$ to $8.91\%$ at a tenth of the budget.
