...
those counts is the \emph{primary} contrast, because recording a
non-reacher at $1.0$ can never flip its sign --- such an arm always
registers as later. $\Delta$RMTT, the paired difference in restricted mean
time, is reported second: that same recording understates a non-reacher's
true time and therefore flatters the seeded arm. Both are Holm-corrected
over the six seeded arms of each solver. The control reaches its own target
...
\multirow{4}{*}{GA}
 ...
 & MIX & 77\%  & 18/37 & 0.015    & 0.70 & $+0.09$ $[+0.04,+0.13]$ \\
\midrule
\multirow{4}{*}{ABCE3}
 & A0  & 100\% & ---   & ---      & 0.38 & --- \\
 & V2  & 100\% & 54/2  & $<0.001$ & 0.16 & $-0.22$ $[-0.28,-0.16]$ \\
 & MOR & 70\%  & 13/40 & $<0.001$ & 0.60 & $+0.22$ $[+0.14,+0.30]$ \\
...
\multirow{4}{*}{TSN2}
 ...
 & MOR & 43\%  & 19/41 & 0.031    & 0.74 & $+0.22$ $[+0.12,+0.31]$ \\
 & MIX & 93\%  & 42/15 & 0.003    & 0.46 & $-0.06$ $[-0.11,+0.01]$ \\
\bottomrule
\end{tabular}
\end{table}
...
The other two solvers give the opposite answer, and the GA gives it
emphatically. \emph{Every one of its six seeded arms is significantly
slower} to the control's own final quality, MIX included (18 earlier, 37
later, $p=0.015$), and both contrasts agree on all six. On fEABCLS the
three competitive pools --- V2H, V2 and MIX --- are statistically
indistinguishable from the control ($p=1.00$ for V2 and MIX), while the
three weak ones are significantly slower. Seeding buys that solver nothing
in time, and it costs the GA time on every generator we tried.

Mean and sign now agree almost everywhere; they part company on two arms
only, V2H on fEABCLS and GT on TSN2, where $\Delta$RMTT reaches
significance and the sign test does not. The reading in those two cases is
that the gain is concentrated in a minority of instances rather than being
a general shift, which is what the sign test is there to detect.

Thirteen arm--solver pairs are significantly \emph{slower} to the target
than their control, and MOR is the extreme of every one: it is slower on
all four solvers and reaches the target on no instance at all on the GA and
fEABCLS, where 59 of 61 comparisons go against it. GT is slower on three
solvers. Both sit at the bottom of the quality ladder of
Section~\ref{sec:res-seedqual}, so this is the one place in the study where
seed quality and outcome move together.
