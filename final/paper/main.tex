...
excess over the lower bound by 37\% to 53\% on all four solvers, equivalent
to 4\% to 11\% of makespan.
...
lower bounds by 37\% to 53\% on all four solvers, including the one it
...
TSN2 --- reductions of 53\%, 46\%, 41\% and 37\% in the excess over the
lower bound, equivalent to 11\%, 9\%, 6\% and 4\% of makespan. The margins
narrow but do not vanish: at the budget limit MIX still leads on all four. The practical
...
\multirow{4}{*}{GA}
 & A0  & 100\% & ---   & ---      & 0.96 & --- \\
 & V2  & 34\%  & 16/28 & 0.48     & 0.86 & $-0.10$ $[-0.16,-0.04]$ \\
 & MOR & 15\%  & 8/34  & $<0.001$ & 0.95 & $0.00$ $[-0.04,+0.03]$ \\
 & MIX & 51\%  & 25/24 & 1.00     & 0.82 & $-0.14$ $[-0.22,-0.07]$ \\
\midrule
\multirow{4}{*}{ABCE3}
 & A0  & 100\% & ---   & ---      & 0.83 & --- \\
 & V2  & 92\%  & 55/5  & $<0.001$ & 0.30 & $-0.53$ $[-0.61,-0.44]$ \\
 & MOR & 61\%  & 25/27 & 0.89     & 0.78 & $-0.05$ $[-0.12,+0.02]$ \\
 & MIX & 89\%  & 52/6  & $<0.001$ & \textbf{0.29} & $\mathbf{-0.54}$ $[-0.63,-0.45]$ \\
\midrule
\multirow{4}{*}{fEABCLS}
 & A0  & 100\% & ---   & ---      & 0.90 & --- \\
 & V2  & 52\%  & 28/22 & 0.48     & 0.78 & $-0.12$ $[-0.19,-0.06]$ \\
 & MOR & 15\%  & 6/41  & $<0.001$ & 0.96 & $+0.05$ $[+0.02,+0.09]$ \\
 & MIX & 54\%  & 33/20 & 0.30     & 0.74 & $-0.16$ $[-0.23,-0.09]$ \\
\midrule
\multirow{4}{*}{TSN2}
 & A0  & 100\% & ---   & ---      & 0.87 & --- \\
 & V2  & 46\%  & 24/33 & 0.87     & 0.82 & $-0.05$ $[-0.11,+0.01]$ \\
 & MOR & 31\%  & 18/37 & 0.072    & 0.83 & $-0.04$ $[-0.10,+0.02]$ \\
 & MIX & 66\%  & 38/18 & 0.063    & 0.71 & $-0.16$ $[-0.23,-0.09]$ \\
\bottomrule
\end{tabular}
\end{table}
...
Under this treatment ABCE3 stands alone. Its mixed pool reaches the
control's \emph{final} quality earlier on 52 of the 58 instances that are
not ties, cutting restricted mean time from $0.83$ to $0.29$ of the budget;
V2 does almost as well, and five of the six seeded arms are significant on
both contrasts. These are unambiguous ``same quality, sooner'' results, and
they agree with the converged-quality finding for that solver.

No other solver supports that claim. On the GA the mixed pool is earlier on
25 instances and later on 24 --- a coin flip. On fEABCLS and TSN2 no arm
clears the sign test after correction, TSN2's mixed pool coming closest at
$p=0.063$. The two contrasts disagree on exactly these three solvers:
$\Delta$RMTT is negative and significant for four GA arms, three fEABCLS
arms and two TSN2 arms whose sign tests are not. That pattern has a
definite reading. A mean is pulled down by a minority of instances on which
the arm arrives much earlier, while the majority do not improve at all; the
sign test is what separates a general acceleration from one concentrated in
a subset. Outside ABCE3, seeding buys the second, and the distinction
matters to a practitioner choosing whether to seed a given run.

Three arms are significantly \emph{slower} to the target than their
control: MOR on the GA (8 earlier, 34 later), and MOR (6/41, $\Delta$RMTT
$+0.05$) and GT (10/38) on fEABCLS. MOR is the weakest generator on the
quality ladder of Section~\ref{sec:res-seedqual}, so this is the one place
...
test: its mixed pool is earlier on 33 instances and later on 20
(Section~\ref{sec:res-ttt}). What remains is the budget-fraction table,
where fEABCLS improves from $15.87\%$ to $9.38\%$ at a tenth of the budget.
