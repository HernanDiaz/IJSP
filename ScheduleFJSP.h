...
public:
	...
	* Main constructor
	*/
	ScheduleFJSP(const ProblemFJSP * problem = NULL);

	/**
	* Copy constructor
	*/
	ScheduleFJSP(const ScheduleFJSP & source);
...
public:
	...
	* Assignment overload
	*/
	ScheduleFJSP & operator=(const ScheduleFJSP & source);
	...
	*/
	const TaskFJSP * operator[](const unsigned int index) const;
