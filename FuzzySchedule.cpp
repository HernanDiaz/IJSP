...
//=============================================================================
//====  Main constructor  =====================================================
FuzzySchedule::FuzzySchedule(const FuzzyProblem * problem) {
...
//====  Copy constructor  =====================================================
FuzzySchedule::FuzzySchedule(const FuzzySchedule & source) {
...
std::vector<int> & FuzzySchedule::getTaskOrder() {
	if (!this->isSorted)
...
TFN FuzzySchedule::getCTMachine(const unsigned int machine) const {
...
TFN FuzzySchedule::getCTJob(const unsigned int job) const {
...
//=============================================================================
//====  Assignment overload  ==================================================
FuzzySchedule & FuzzySchedule::operator=(const FuzzySchedule & source) {
	this->nScheduledTasks = source.nScheduledTasks;
	this->tailsUpdated = source.tailsUpdated;
	this->isSorted = source.isSorted;
...
//=============================================================================
//====  addTask Method  =======================================================
void FuzzySchedule::addTask(const int taskIdx, TFN & ST, const int macSuc) {

	const FuzzyTask * task = (*(this->problem))[taskIdx];
	int mac = task->machine;
	int job = task->job;
	int macPred;
	TFN::Compare cev = TFN::C_EV;
	...
	if (this->isSorted) {
		...
		for (i = this->nScheduledTasks; i > 0; i--) {
			...
			if (this->taskInfo[this->taskOrder[i]].head.isLesserThan(ST, cev)) {
...
//====  updateTails Method  ===================================================
void FuzzySchedule::updateTails(const TFN::Maximum maxType) {
	...
	while (taskQueue.size() > 0) {
		...
		else this->taskInfo[taskIdx].tail = TFN(0, 0, 0);
...
void FuzzySchedule::reset() {
	...
	for (size_t i = 0; i < this->taskInfo.size(); i++) {
		this->taskInfo[i].head = TFN(-1, -1, -1);
		this->taskInfo[i].tail = TFN(0, 0, 0);
...
void FuzzySchedule::updateTopologicalOrder() {
	if (this->isSorted)
		return;
	this->quicksortTasks(0, this->nScheduledTasks - 1, new Random());
	this->isSorted = true;
}


//====  apply quicksort to sort tasks  ========================================
void FuzzySchedule::quicksortTasks(int left, int right, Random *rng) {
	int pivot;
	TFN pivotValue;
	TFN::Compare cp = TFN::C_EV;
	...
	for (int i = left; i < right; i++) {
		if (this->taskInfo[this->taskOrder[i]].head.isLesserThan(pivotValue, cp)) {
			std::swap(this->taskOrder[i], this->taskOrder[pivot]);
			pivot++;
		}
		else if (this->taskInfo[this->taskOrder[i]].head.isEqualTo(pivotValue, cp)
