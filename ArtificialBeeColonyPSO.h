...
// I-018 and I-021, accepted 2026-09-24. Which individual the second
// local-search call of MALS_SOME reaches. Every food source makes a pair of
// children and MALS_SOME is called on that pair: it searches the better child,
// then makes a second call.
//   "index"  (default, the original code): individual i, the loop counter,
//            which with a pair always lands on child 0 -- the better child a
//            little over half the time.
//   "best"   : always the better child again, a second tabu search from its
//            local optimum with an empty tabu list (I-018: -3.38 on the
//            per-instance mean, 18 of 21, p = 0.0010).
//   "best-repeat" : like "best", then again for as long as the last call
	...
	*/
	...
	protected:
		...
		*/
		...
		// I-018 / I-021: the switch and the mechanism's counters.
		bool lsPickBest;
		bool lsPickRepeat;
		unsigned long lsSecondOnBest;     // second calls on the better child
