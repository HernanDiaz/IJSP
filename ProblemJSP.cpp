...
namespace JSP {
	...
	//=============================================================================
	//====  Default constructor  ==================================================
	ProblemJSP::ProblemJSP(const FuzzyFW::ParameterDB *params)
	...
	//====  Main constructor  =====================================================
	ProblemJSP::ProblemJSP(const char *inputFile)
	...
	//====  Setup method  =========================================================
	void ProblemJSP::setup(const FuzzyFW::ParameterDB *params) {
		...
		if (paramValue.compare("YES") == 0)
			...
			errorMsg += paramValue + "\'";
			throw JSPException("Loading problem", errorMsg);
		}
	...
	//====  Copy constructor  =====================================================
	ProblemJSP::ProblemJSP(const ProblemJSP & source)
		: Problem(source) {
		...
		for (size_t t = 0; t < source.task.size(); t++)
			this->task.push_back(new TaskJSP(*source.task[t]));
	...
	//=============================================================================
	//====  Get Number of Tasks  ==================================================
	unsigned int ProblemJSP::getNumberTasks(const unsigned int job) const {
		...
		throw JSPException("Problem", errorMsg);
	}


	//====  Get the different bounds  =============================================
	double ProblemJSP::getMakespanLB() const {
		if (this->withBounds)
			return this->lb_Makespan;
		return 0.0;
	}

	double ProblemJSP::getAIminUB() const {
		if (this->withBounds)
			return this->ub_AImin;
		return 1.0;
	}

	double ProblemJSP::getAIavgUB() const {
		if (this->withBounds)
	...
	//====  Get Duedate  ==========================================================
	FuzzyFW::TimeWindow * ProblemJSP::getTimeWindow(const unsigned int job)
		const {
		if (job < this->nJobs && job >= 0) {
	...
	//====  Get Task in a job  ====================================================
	int ProblemJSP::getTaskId(const unsigned int job, const int position)
		const {
		if (job >= this->nJobs || job < 0) {
			std::string errorMsg = "Trying to access unexisting job: ";
			errorMsg += valueToString(job);
	...
	//====  Index access  =========================================================
	TaskJSP * ProblemJSP::getTask(const unsigned int taskId) const {
	...
	//=============================================================================
	//====  Load data from file  ==================================================
	void ProblemJSP::loadFile(const char *inputFile) {
		std::string reader;
		std::stringstream lineStream;
		std::ifstream input;
		std::vector<TaskJSP *> preTasks;
		unsigned int machine, lastTask, job;
		size_t jobSize;
		FuzzyFW::Crisp duration;
		...
		for (unsigned int i = 0; i < this->nJobs; i++) {
			...
			while (lineStream >> machine) {
				preTasks.push_back(new TaskJSP(preTasks.size(),
					i, machine));
			}
			lineStream.clear();
		}
	...
	//=============================================================================
	//====  Save data to file  ==================================================
	void ProblemJSP::saveFile(const char *outputFile) {
		std::ofstream output;
	...
	void ProblemJSP::loadDueDates(std::ifstream &input) {
		std::string reader;
	...
	void ProblemJSP::loadBounds(std::ifstream &input) {
		std::string reader;
