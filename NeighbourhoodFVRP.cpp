...
//=============================================================================
//-----  Copy constructor  ----------------------------------------------------
NB_Vidal_FVRP_FD::NB_Vidal_FVRP_FD(const NB_Vidal_FVRP_FD & source)
...
//-----  Setup method  --------------------------------------------------------
void NB_Vidal_FVRP_FD::setup(FuzzyFW::ParameterDB *parameters) {
...
//-----  Destructor  ----------------------------------------------------------
NB_Vidal_FVRP_FD::~NB_Vidal_FVRP_FD() {
...
void NB_Vidal_FVRP_FD::setInitialSolution(FuzzyFW::Solution *solution,
	...
	this->currentFitness =
		dynamic_cast<FuzzyFW::FitnessTFN *>(fitness);
	if (this->currentFitness == NULL) {
	...
	for (unsigned int i = 0; i <= problem->getNumberCustomers(); i++) {
		this->durationMatrix[i].resize(problem->getNumberCustomers(),
			FuzzyFW::TFN(0, 0, 0));
	...
	for (unsigned int i = 0; i < this->route->getNumberVehicles(); i++) {
		...
		this->durationMatrix[current][current] = problem->getServiceTime(current);

		this->durationMatrix[current][0] = problem->getTravelTime(current, 0)
			+ problem->getServiceTime(current);
		this->stockMatrix[current][0] = problem->getDemand(current);
		...
		while (current != 0) {
			this->durationMatrix[pred][pred] = problem->getServiceTime(pred);
			this->segments[pred][pred] = pred;
			for (size_t j = visitedNodes.size() - 1; j >= 0; j++) {
				this->durationMatrix[pred][visitedNodes[j]] = problem->getTravelTime(pred, current)
					+ problem->getServiceTime(pred) + this->durationMatrix[current][visitedNodes[j]];
				this->stockMatrix[pred][visitedNodes[j]] = problem->getDemand(pred)
					+ this->stockMatrix[current][visitedNodes[j]];
				this->distanceMatrix[pred][visitedNodes[j]] = problem->getDistance(pred, current)
...
//-----  Find enighbours  -----------------------------------------------------
unsigned int NB_Vidal_FVRP_FD::findNewNeighbours(const FuzzyFW::SharedVars *svars) {

	unsigned int pos1, pos2;
	unsigned int start1, end1, start2, end2;
	FuzzyFW::TFN currentCost;
...
FuzzyFW::Fitness * NB_Vidal_FVRP_FD::evaluateNeighbour(const unsigned int idx,
	...
	this->neighbours[idx]->setEvaluation(newRoute,
		new FuzzyFW::FitnessTFN(cost, false));
...
void NB_Vidal_FVRP_FD::acceptNeighbour(const unsigned int idx,
	...
	for (size_t t = 0; t < vehicles.size(); t++) {
		...
		while (current != 0) {
			...
			for (size_t j = visitedNodes.size() - 1; j >= 0; j++) {
				this->durationMatrix[pred][visitedNodes[j]] = problem->getTravelTime(pred, current)
					+ problem->getServiceTime(pred) + this->durationMatrix[current][visitedNodes[j]];
				this->stockMatrix[pred][visitedNodes[j]] = problem->getDemand(current)
					+ this->stockMatrix[current][visitedNodes[j]];
				this->distanceMatrix[pred][visitedNodes[j]] = problem->getDistance(pred, current)
					+ this->distanceMatrix[current][visitedNodes[j]];
...
//-----  Get the estimation  --------------------------------------------------
FuzzyFW::Fitness * NB_Vidal_FVRP_FD::getEstimation(const unsigned int idx,
	const FuzzyFW::SharedVars *svars) {
	...
	if (neigh->vehicle1 == neigh->vehicle2) {
		...
		cost = cost + problem->getTravelTime(end, start);
		end = neigh->final2;
	...
	for (unsigned int i = 0; i < this->route->getNumberVehicles(); i++) {
		if (i != neigh->vehicle1 && i != neigh->vehicle2)
			cost = cost + this->route->getRouteTime(i);
	}

	this->neighbours[idx]->setEstimatedQuality(new FuzzyFW::FitnessTFN(cost));
...
//-----  Discard a neighbour  --------------------------------------------------
void NB_Vidal_FVRP_FD::discardNeighbour(const unsigned int idx) {
	if (idx < 0 || idx >= this->numNeighbours || this->neighbours[idx] == NULL) {
		std::string errorMsg = "Trying to acces a non-existing neighbour";
		throw new FVRPException("Neighbourhood", errorMsg);
	}
...
FuzzyFW::Neighbour* NB_Vidal_FVRP_FD::getNeighbour(const unsigned int idx) {
	if (idx < 0 || idx >= this->numNeighbours || this->neighbours[idx] == NULL) {
		std::string errorMsg = "Trying to acces a non-existing neighbour";
		throw new FVRPException("Neighbourhood", errorMsg);
	}
...
void NB_Vidal_FVRP_FD::sortByEstimation(
	const FuzzyFW::SharedVars *svars) {
...
//-----  Quick sort  ----------------------------------------------------------
void NB_Vidal_FVRP_FD::quickSort(const int left, const int right,
	FuzzyFW::Random *rng) {
...
double NB_Vidal_FVRP_FD::getDemandMetric(FuzzyFW::TFN demand, double capacity) {
