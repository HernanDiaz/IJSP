...
//=============================================================================
//=====  Default constructor  =================================================
SGS_Split_TW_FD_Time::SGS_Split_TW_FD_Time(const FuzzyFW::ParameterDB *params)
...
//=====  Copy constructor  ====================================================
SGS_Split_TW_FD_Time::SGS_Split_TW_FD_Time(const SGS_Split_TW_FD_Time &source)
...
//=====  Setup method  ========================================================
void SGS_Split_TW_FD_Time::setup(const FuzzyFW::ParameterDB *params) {
...
RouteFVRP * SGS_Split_TW_FD_Time::buildPlan(
	...
	for (size_t i = 0; i < order.size(); i++) {
		visitTime = getCrispValue(fuzzyProb->getTravelTime(0, order[i]));
		routeTime = this->getVisitTime(visitTime, fuzzyProb->getTimeWindow(order[i]));

		routeCost = routeTime
			+ getCrispValue(fuzzyProb->getServiceTime(order[i]))
			+ getCrispValue(fuzzyProb->getTravelTime(0, order[i]));
		demand = fuzzyProb->getDemand(order[i]);

		dest = i + 1;
		while (dest < fuzzyProb->getNumberCustomers()) {
			visitTime = routeCost - getCrispValue(fuzzyProb->getTravelTime(0, order[dest - 1]))
				+ getCrispValue(fuzzyProb->getTravelTime(order[dest - 1], order[dest]));
			routeTime = this->getVisitTime(visitTime, fuzzyProb->getTimeWindow(order[i]));

			routeCost = routeTime
				+ getCrispValue(fuzzyProb->getServiceTime(order[dest]))
				+ getCrispValue(fuzzyProb->getTravelTime(0, order[dest]));
			...
			if (compareDouble(getDemandMetric(demand, fuzzyProb->getMaxCapacity()),
				...
				&& compareDouble(routeCost, fuzzyProb->getTimeHorizon() <= 0)) {
				if (i == 0
					|| compareDouble(minCost[dest], minCost[i - 1] + routeCost) > 0) {
...
//=====  Schedule a task  =====================================================
FuzzyFW::TFN SGS_Split_TW_FD_Time::addCustomer(ProblemFVRP *problem,
	...
	crispVT = getCrispValue(visitTime);
	crispVT = this->getVisitTime(crispVT, problem->getTimeWindow(customerId));
	visitTime = FuzzyFW::TFN(crispVT, crispVT, crispVT);
...
double SGS_Split_TW_FD_Time::getDemandMetric(FuzzyFW::TFN demand, double capacity) {
...
//=====  Get the minimu visit time  ===========================================
double SGS_Split_TW_FD_Time::getVisitTime(double visitTime,
	const FuzzyFW::TimeWindow *tw) {
	double vt = visitTime;

	// Extend for the rest in future versions
	if (tw->CRISP) {
		const FuzzyFW::TimeWindowCrisp *window =
			dynamic_cast<const FuzzyFW::TimeWindowCrisp *>(tw);
		vt = std::max(vt, window->earlyTime);
		return vt;
	}
	return vt;
}
...
//=============================================================================
//=====  Build plan  ==========================================================
RouteFVRP * SGS_Split_TW_FD_Distance::buildPlan(
	const FuzzyFW::SharedVars * const svars, std::vector<int> &order) {

	unsigned int currentVehicle = 0;
	double routeCost = 0.0, routeDist;
	double visitTime, routeTime;
	FuzzyFW::TFN demand;
