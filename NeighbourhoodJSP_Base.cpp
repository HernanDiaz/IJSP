...
//-----  Copy constructor  ----------------------------------------------------
NB_ParallelBase_MakespanJSP::NB_ParallelBase_MakespanJSP(
	const NB_ParallelBase_MakespanJSP &source)
	...
	currentFitness(NULL)
{
	for (size_t i = 0; i < source.neighbours.size(); i++) {
		if (source.neighbours[i] != nullptr)
			neighbours.push_back(std::make_unique<NeighbourJSP_Arc>(*source.neighbours[i]));
		else
...
//-----  Setup method  --------------------------------------------------------
void NB_ParallelBase_MakespanJSP::setup(FuzzyFW::ParameterDB *parameters) {
...
void NB_ParallelBase_MakespanJSP::setInitialSolution(
	FuzzyFW::Solution *solution, FuzzyFW::Fitness *fitness,
	const FuzzyFW::SharedVars *svars) {

	if (this->schedule != NULL)
		delete this->schedule;
	this->schedule = dynamic_cast<ScheduleJSP *>(solution);
	if (this->schedule == NULL) {
	...
	this->currentFitness = dynamic_cast<FuzzyFW::FitnessCrisp *>(fitness);
	if (this->currentFitness == NULL) {
	...
	ProblemJSP *problem = dynamic_cast<ProblemJSP *>(svars->problem);
	if (problem == NULL) {
		std::string errorMsg = "This negihbourhood can be applied only";
		errorMsg += " to JSP problems";
		throw JSPException("Neighbourhood", errorMsg);
	}
...
FuzzyFW::Fitness *NB_ParallelBase_MakespanJSP::getEstimation(
	...
	if (!this->neighbours[idx]->isEstimated()) {
		if (this->estimator == ESTIM_HEADTAILS)
			...
			// No estimator: give the neighbour the worst possible estimate so
			// it is never preferred on the strength of a guess. The bound is
			// the integer one now; it is only ever compared, never added to.
			this->neighbours[idx]->setEstimatedQuality(
				new FuzzyFW::FitnessCrisp(
					FuzzyFW::Crisp(std::numeric_limits<int>::max()), false));
	}
...
//-----  Discard a neighbour  -------------------------------------------------
void NB_ParallelBase_MakespanJSP::discardNeighbour(const unsigned int idx) {
...
void NB_ParallelBase_MakespanJSP::estimateHeadsTails(const unsigned int idx) {
	if (idx < 0 || idx >= this->numNeighbours) {
		std::string errorMsg = "Trying to acces a non-existing neighbour";
		throw JSPException("Neighbourhood", errorMsg);
	}
...
//-----  Sort by estimation  --------------------------------------------------
void NB_ParallelBase_MakespanJSP::sortByEstimation(
	const FuzzyFW::SharedVars *svars) {
...
//-----  Quick sort  ----------------------------------------------------------
void NB_ParallelBase_MakespanJSP::quickSort(const int left, const int right,
	const FuzzyFW::SharedVars *svars) {
...
FuzzyFW::Neighbour *NB_ParallelBase_MakespanJSP::getNeighbour(
