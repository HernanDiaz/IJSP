...
connectivity analysed for the bi-objective setting. Experimental results show
that the lexicographic tie-break alone recovers 7.1\% of the idle energy at
zero makespan cost with respect to the published makespan-only tabu search,
that timing optimisation contributes a further ${\sim}10\%$, and that both
levers are complementary. A dominance-based memetic algorithm returning
Pareto-front approximations completes the study.
\end{abstract}
...
published makespan-only tabu search. On the classical instances the
tie-break recovers \textbf{7.1\% of the idle energy} ($p=0.003$) with no
degradation of the primary objective (makespan $-0.19\%$; we attribute
this small difference to machine-load conditions rather than to the
tie-break and make no claim of improvement).
...
$15{\times}15$ & 10 & \multicolumn{9}{c}{\emph{running}} \\
$20{\times}15$ -- $50{\times}20$ & 60 & \multicolumn{9}{c}{\emph{running}} \\
\bottomrule
\end{tabular}
\end{table}

\todo{Fill Taillard rows when the full tier finishes
(\texttt{analyze\_lexme.py}, \texttt{RS=heur}); add per-group Wilcoxon
$p$-values and the grand mean.}

\subsection{Order vs.\ timing: decomposing the energy gains}
\label{sec:exp-decomp}

The two levers are complementary. On the classical group: the tie-break
(order) contributes $-7.1\%$; right-shifting the baseline's solutions
(timing only) contributes $-10.5\%$; combining both yields
$\mathbf{-15.9\%}$ total idle energy at zero makespan cost. Crucially, the
lexicographic advantage \emph{survives} timing optimisation: after
right-shifting both arms, LexME remains $-6.1\%$ below the baseline
($p=0.003$). The tie-break therefore finds structurally better orders, not
merely orders whose slack timing could recover anyway.
...
\todo{Small-instance validation: measure how close each arm's best
solutions get to the exact $\varepsilon$-constraint fronts (ft10 and the
additional probes); include the front figure with the lexicographic point
marked.}
...
energy hidden in makespan plateaus (7.1\% on classical instances, at zero
makespan cost); right-shift timing optimisation, which contributes a
further ${\sim}10\%$ and is near-optimally approximated by a single
backward pass; and the bi-critical neighbourhood $N_2^{ME}$, the first
