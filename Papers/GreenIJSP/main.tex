...
%% Remaining \todo{...} boxes flag only a front figure, the epsilon-indicator
%% vs. exact fronts, the tuning-gain supplement, and acknowledgements.
%%
...
machine power data and develop two complementary treatments on top of a
memetic algorithm with tabu search. For the single-solution setting we
propose a progression of methods: (i) a lexicographic strategy that
...
connectivity analysed for the bi-objective setting. For the Pareto setting we
propose a \emph{capped $\varepsilon$-ladder}: rather than covering the front
with a single population, it decomposes the front into makespan-capped
single-objective energy problems, solved by the same tabu machinery and
chained by warm-starting --- turning an existing single-objective
interval-JSP solver into a front generator, with soundness and
capped connectivity established analytically.
Experimentally, on the full 82-instance benchmark the lexicographic tie-break
alone recovers 7.9\% of the idle energy on average --- up to 26.5\% on the
largest job/machine-ratio instances --- at no makespan cost with respect to
the published makespan-only tabu search; timing optimisation contributes a
further ${\sim}8\%$; and the two levers are complementary (combined
${\sim}15\%$). For the front, standard multiobjective evolutionary algorithms
--- an NSGA-II memetic and a Pareto-archive ABC --- serve as controls rather
than rivals: they show that a good front does not come for free, and that
decomposition exploits a fixed budget better than dominance-based search
across most of the instance space (highest hypervolume on 55 of 82 instances,
dominant up to $20{\times}20$), while we also delineate the regime where the
scalar anchor loses traction on the largest high-ratio instances and
dominance-based search becomes competitive again.
\end{abstract}
...
makespan-capped energy problems. The design is backed by proof
(goal-programming and $\varepsilon$-constraint equivalence, capped
connectivity) rather than assembled ad hoc.
...
within the basin already reached.
...
Section~\ref{sec:conflict}); we also run per-run paired Wilcoxon tests.
...
MA-P and $0.703$ for ABC-P), and per-run Wilcoxon tests find it significantly
better than ABC-P on $48/82$ and than MA-P on $24/82$ instances. On the
...
the last block reports per-run paired-Wilcoxon significant wins of the ladder
($\alpha=0.05$). Bold: best group mean.}
\label{tab:mo-full}
\begin{tabular}{l r ccc c cc}
\toprule
 & & \multicolumn{3}{c}{mean HV ratio} & wins & \multicolumn{2}{c}{L $>$ (Wilcoxon)} \\
\cmidrule(lr){3-5}\cmidrule(lr){7-8}
Group & \# & LADDER & ABC-P & MA-P & L/A/M & ABC-P & MA-P \\
\midrule
classical      & 12 & \textbf{0.875} & 0.640 & 0.663 & 10/2/0 & 9 & 8 \\
tai $15{\times}15$ & 10 & \textbf{0.960} & 0.750 & 0.756 & 10/0/0 & 8 & 6 \\
tai $20{\times}15$ & 10 & \textbf{0.945} & 0.734 & 0.768 & 9/1/0  & 6 & 4 \\
tai $20{\times}20$ & 10 & \textbf{0.965} & 0.769 & 0.752 & 10/0/0 & 8 & 2 \\
tai $30{\times}15$ & 10 & \textbf{0.888} & 0.721 & 0.705 & 8/1/1  & 4 & 2 \\
tai $30{\times}20$ & 10 & 0.785 & \textbf{0.923} & 0.693 & 0/9/1  & 1 & 0 \\
tai $50{\times}15$ & 10 & \textbf{0.876} & 0.522 & 0.818 & 5/1/4  & 3 & 1 \\
tai $50{\times}20$ & 10 & 0.828 & 0.579 & \textbf{0.854} & 3/2/5 & 9 & 1 \\
\midrule
\textit{All 82} & 82 & \textbf{0.890} & 0.703 & 0.749 & 55/16/11 & 48 & 24 \\
\bottomrule
\end{tabular}
\end{table}
...
\todo{Still to add: a representative front figure (ladder vs.\ the two
controls on one small and one large instance), the additive
$\varepsilon$-indicator against the exact fronts on the probed instances, and
the full tuned-vs-inherited tuning-gain table as supplementary material.}
...
For the Pareto front, our central methodological claim is not that we beat
NSGA-II, but that an existing single-objective interval-JSP solver can be
recycled, unchanged, into a strong front generator by decomposing the front
into warm-started, makespan-capped energy problems. With two off-the-shelf
