...
\subsection{Front generation: dominance-based baselines}
\label{sec:pareto-baselines}

The natural way to return a front is a dominance-based multiobjective
evolutionary algorithm. Dominance is defined over the product of the two
LEX2 total orders: $\sigma$ dominates $\sigma'$ iff
...
(non-dominated sorting plus crowding distance in the replacement),
following the fuzzy-JSP design of \cite{Afsar2022}. The archive stores the
...
the cap region. Section~\ref{sec:ladder} gives the theoretical backing:
...
whole anchor-plus-levels pipeline) and returns the negated hypervolume of
its front against a fixed per-instance reference point, over the same
stratified 20-instance training subset used in \cite{DiazTabu2026}. The
...
\subsection{Front quality: the capped ladder vs.\ dominance baselines}
\label{sec:exp-pareto}

Table~\ref{tab:mo} compares the three tuned arms on the classical
instances (30 runs each). Against a per-instance reference front (the
non-dominated union of all arms), the ladder attains the highest
hypervolume ratio on $10$ of $12$ instances (mean $0.875$, vs.\ $0.663$
for MA-P and $0.640$ for ABC-P). Per-run paired Wilcoxon tests confirm the
advantage: the ladder is significantly better than each baseline on
$8$ of $12$ instances, and is significantly beaten on only one
(\texttt{abz9}, by ABC-P). Tuning narrows but does not close the gap ---
ABC-P improves from a mean ratio of $0.54$ (inherited configuration) to
$0.64$ --- so the ladder's advantage is a property of the method, not of a
favourable parameterisation.

\begin{table}[htb]
\centering\small
\caption{Front hypervolume ratio (vs.\ the per-instance union reference;
higher is better) of the three tuned arms on the classical instances,
30 runs, best-over-runs front. Bold: best per row.}
\label{tab:mo}
\begin{tabular}{l ccc}
\toprule
...
\todo{Extend Table~\ref{tab:mo} to the 70 Taillard instances (tuned runs
launched); add HV/$\varepsilon$-indicator vs.\ the exact fronts on the
probed instances and a front figure. Report the tuning-gain analysis
(tuned vs.\ inherited) as a secondary table.}
...
near-optimally approximated by a single backward pass; the bi-critical
neighbourhood $N_2^{ME}$, the first sequencing neighbourhood targeting
idle gaps, with feasibility proven and connectivity established for
makespan and machine-completion optimality; and a capped
$\varepsilon$-ladder that generates the full Pareto front by concentrating
recombination-driven search on one makespan level at a time. Tuned
separately with irace, the ladder attains higher hypervolume than both a
plain Pareto-archive engine and an NSGA-II memetic (significantly on
$8/12$ classical instances), showing that decomposing the biobjective
problem into warm-started single-objective levels outperforms covering the
whole front at once. The design is backed throughout by either proof
...
recombination). Future work: the flexible interval JSP, and vague-goal
green scheduling~\cite{GarciaGomez2026}.
