#!/usr/bin/env bash
# Replicacion GA: 30 runs sobre las 12 instancias clasicas, parsea best/avg RE vs LB.
cd "$(dirname "$0")/.."
SET=repro/setup_ga.txt
OUT=repro/out_ga
mkdir -p "$OUT"

# instancia_archivo  LB  etiqueta
read -r -d '' ROWS <<'EOF'
...
while read -r file lb label; do
  ...
  if [ -z "$sols" ]; then echo "$label: SIN SALIDA"; continue; fi
  awk -F';' -v lb="$lb" -v label="$label" '
    ...
    END {
      ...
      printf "%-6s %5s  %6.1f (%4.1f%%)  %6.2f (%4.1f%%)   [n=%d]\n",
