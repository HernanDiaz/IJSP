/*
 * NeighbourhoodJSP_N2Inter.cpp
 *
 *  Created on: Oct 12, 2019
 *      Author: hdiaz
 */

#include "NeighbourhoodJSP_N2Inter.h"
#include <iostream>
namespace JSP {
...
unsigned int NB_ParallelN2Inter_MakespanJSP::findNewNeighbours(
	...
	// jsp.makespan.n2inter is the same neighbourhood as jsp.makespan.n2.
	...
	for (unsigned int x = 0; x < nTasks; x++) {
		if (boundary[x] >= 0) {
			...
			if (this->numNeighbours < this->neighbours.size()
				...
				this->neighbours.push_back(std::make_unique<NeighbourJSP_Arc>(x, y));
			this->numNeighbours++;
		}
	}
