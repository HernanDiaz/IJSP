...
#
# run_exp7.sh
# Experiment 7: Full comparison of 5 neighbourhoods with irace-tuned HC configs.
# 5 neighbourhoods x 82 instances x 30 runs.
# Skips already-completed experiments (safe to restart).
#
# Usage (from WSL): bash run_exp7.sh
#
...
RESULTS_BASE="${SCRIPT_DIR}/results/exp7"
LOG_FILE="/home/diazhernan/run_exp7.log"

MAX_PARALLEL=24

# Experiment 7 configurations: folder_name -> setup_file
...
echo "======================================================" | tee "$LOG_FILE"
echo " IJSP Experiment 7 — irace-tuned HC neighbourhood comparison" | tee -a "$LOG_FILE"
...
mkdir -p "${RESULTS_BASE}/exp7_logs"

# Read instance paths (exclude tai100 only — tai50 included, same as Exp4)
...
for i in "${!PENDING_SETUP[@]}"; do
    ...
    cfg_name="${setup##*/setup_}"; cfg_name="${cfg_name%_hc_tuned.txt}"
    per_log="${RESULTS_BASE}/exp7_logs/${cfg_name}__${inst_name}.log"
...
ELAPSED=$(( $(date +%s) - START_TIME ))
echo "" | tee -a "$LOG_FILE"
echo "=== Experiment 7 finished ===" | tee -a "$LOG_FILE"
