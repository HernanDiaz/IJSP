#!/usr/bin/env python3
"""I-023: the pre-declared analysis.
...
0. MECHANISM CHECK, printed first and read first, in two halves, because the
   ...
   tested:
     a) the kicks and how many of the kicked copies end better than the
        stuck child: about a third in the pre-launch check, none in the
        control;
     b) the generations per run, the price: under half in the pre-launch
        check.
...
Filter rule: discard if the mean of (kick - control) over the four filter
...
STATS = ["LS kicks at the end of a chain",
         "LS kicked copies that ended better",
...
def directories(tags, cell):
    ...
    for tag in tags:
        out += sorted(glob.glob(os.path.join(
            EXPERIMENT, "results", "I-023_%s_p*_%s" % (tag, cell))))
    return out
...
def report(data, tags, instances, label):
    ...
    print("%-6s %-42s %10s %10s" % ("inst", "statistic", "control", "kick"))
    shares = []
    for instance in instances:
        for key in STATS:
            c = solver_stat(tags, "control", instance, key)
            f = solver_stat(tags, "kick", instance, key)
            if key == STATS[0]:
                shares.append(f)
            print("%-6s %-42s %10.2f %10.2f" % (instance, key, c, f))
    if shares:
        mean_share = statistics.mean(s for s in shares if s == s)
        print("\nkicks per run: %.0f on average (the control makes none)" % mean_share)

    print("\n=== I-023 %s: per-instance mean (primary), best-of-%d (reported) ==="
          % (label, BLOCK))
    print("%-6s %11s %11s %9s   %11s %11s %9s"
          % ("inst", "mean ctrl", "mean kick", "d", "bo5 ctrl", "bo5 kick", "d"))
    mean_d, tail_d = [], []
    for instance in instances:
        c, f = data[("control", instance)], data[("kick", instance)]
...
def main():
    ...
    if which == "filter":
        ...
        print("\nfilter rule, on the MEAN: mean(kick - control) = %+.2f;" % mean)
        ...
        print("\nlook %d of %d, symmetric boundary p <= %.4f (Pocock, six looks)"
              % (last, WAVES, POCOCK))
        print("PRIMARY, per-instance mean: %+.2f, kick better on %d of %d, "
              "W = %s, p = %.4f"
              % (statistics.mean(mean_d), better, len(mean_d), W, p))
        print("reported, deciding nothing: best-of-%d moved %+.2f"
              % (BLOCK, statistics.mean(tail_d)))
        if p <= POCOCK and lower:
            print("-> ACCEPT: the kick crosses on the lower rank at wave %d" % last)
