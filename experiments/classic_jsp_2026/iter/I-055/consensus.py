...
Usage: consensus.py <instance> <list file> <K> <seconds>
"""
import csv
import itertools
import os
...
def main():
    ...
    print("%s K=%d: consensus fixes %d of %d machine pairs (%.1f %%), free %d; group best %d; "
          "subspace best %s, status %s, bound %d, %.0f s"
          % (inst, k, fixed, total, 100.0 * fixed / total, total - fixed, best_ms,
             best, solver.StatusName(status), int(solver.BestObjectiveBound()), time.time() - t0), flush=True)
    if best is not None and best < best_ms:
        path = os.path.join(HERE, "found_K%d_%s_%d_Certificate.csv" % (k, inst, best))
