#!/usr/bin/env python3
"""I-003: the pre-declared analysis of the filter or of the waves so far.
...
discard. Discard if the mean of (kick - control) over the four exceeds +2.0
...
def report(data, instances, label):
    print("=== I-003 %s: per-instance mean (best in brackets) ===" % label)
    print("%-6s" % "inst" + "".join("%18s" % c for c in CELLS) + "%13s" % "kick - ctrl")
    differences = []
    for instance in instances:
        row = ""
        for cell in CELLS:
        ...
        d = (statistics.mean(data[("kick", instance)])
...
def main():
    ...
    if which == "filter":
        ...
        print("\nfilter rule: mean(kick - control) = %+.2f; discard if > %+.1f -> %s"
              % (mean, FILTER_RULE,
        ...
        kick_lower = better * 2 > len(d)
        print("\nlook %d of %d, symmetric boundary p <= %.4f (Pocock, six looks)"
              % (last, WAVES, POCOCK))
        print("kick vs control: mean %+.2f, kick better on %d of %d, W = %s, p = %.4f"
              % (statistics.mean(d), better, len(d), W, p))
        if p <= POCOCK and kick_lower:
            print("-> ACCEPT: kick crosses the boundary on the lower rank at wave %d" % last)
        elif p <= POCOCK and not kick_lower:
