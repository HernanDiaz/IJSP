#!/usr/bin/env python3
"""I-057: break the consensus core one machine at a time (from I-055's consensus.py).

As consensus.py, but the consensus pairs on ONE machine (the fifth argument)
are left free too; every other consensus pair stays fixed.
...
Usage: break_machine.py <instance> <list file> <K> <seconds> <machine>
"""
import csv
import itertools
import os
...
def main():
    ...
    for m, ops in by_machine.items():
        for u, v in itertools.combinations(ops, 2):
            ...
            if len(orders) == 1 and m != free_machine:
                fixed += 1
    ...
    print("%s machine %d freed, K=%d: consensus fixes %d of %d machine pairs (%.1f %%), free %d; group best %d; "
          "subspace best %s, status %s, bound %d, %.0f s"
          % (inst, free_machine, k, fixed, total, 100.0 * fixed / total, total - fixed, best_ms,
             best, solver.StatusName(status), int(solver.BestObjectiveBound()), time.time() - t0), flush=True)
    if best is not None and best < best_ms:
        path = os.path.join(HERE, "found_m%d_%s_%d_Certificate.csv" % (free_machine, inst, best))
