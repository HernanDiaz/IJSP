#!/bin/bash
# I-045: the ABC -> CP-SAT hybrid on the other open 20x20 instances.
# Fixed before running: ta22 ta25 ta26 ta27, the 6 lowest distinct schedules
# of each across the line's results, 75 s of CP-SAT each (I-042 model, 14
# workers, seed 1), on an empty machine.
set -u
cd /opt/ijsp/crisp/IJSP/experiments/classic_jsp_2026
I=iter/I-045
LOG=$I/probe_run.log
echo "=== I-045 probe start at $(date) ===" > $LOG
...
for inst in ta22 ta25 ta26 ta27; do
  python3 iter/I-043/top_schedules.py $inst "results/I-0*/${inst}_*_Certificate.csv" 6 > $I/hints_$inst.txt
  k=0
  while read cert run ms; do
    k=$((k+1))
    echo "--- $inst hint $k: $cert run $run makespan $ms" >> $LOG
    python3 iter/I-042/cpsat_probe.py $inst $cert $run 75 >> $LOG 2>&1
    collect $inst hint$k
  done < $I/hints_$inst.txt
done
echo "=== I-045 probe exit at $(date) ===" >> $LOG
