#!/usr/bin/env python3
"""I-017: the pre-declared analysis.
...
0. MECHANISM CHECK, printed first and read first, in two halves, because the
   ...
   tested:
     a) where the second tabu call goes: in the chosen cell every one must
        reach the other child and none the child already searched, while the
        control splits them about 48 / 52 (measured 2026-09-23);
     b) the generations per run, which prices the fix: the pre-launch check
        showed them falling 10 to 18 %, because a fresh child takes more
        iterations to polish than a local optimum takes to re-search.
...
Filter rule: discard if the mean of (chosen - control) over the four filter
...
STATS = ["LS second call on the other child",
         "LS second call on the searched best",
...
def directories(tags, cell):
    ...
    for tag in tags:
        out += sorted(glob.glob(os.path.join(
            EXPERIMENT, "results", "I-017_%s_p*_%s" % (tag, cell))))
    return out
...
def report(data, tags, instances, label):
    ...
    print("%-6s %-42s %10s %10s" % ("inst", "statistic", "control", "chosen"))
    shares = []
    for instance in instances:
        for key in STATS:
            c = solver_stat(tags, "control", instance, key)
            f = solver_stat(tags, "chosen", instance, key)
            if key == STATS[0]:
                shares.append(f)
            print("%-6s %-42s %10.2f %10.2f" % (instance, key, c, f))
    if shares:
        mean_share = statistics.mean(s for s in shares if s == s)
        print("\nsecond calls reaching the other child per run: %.0f on average"
              % mean_share)
        print("(the control sends about half of them to the child already searched)")

    print("\n=== I-017 %s: per-instance mean (primary), best-of-%d (reported) ==="
          % (label, BLOCK))
    print("%-6s %11s %11s %9s   %11s %11s %9s"
          % ("inst", "mean ctrl", "mean chosen", "d", "bo5 ctrl", "bo5 chosen", "d"))
    mean_d, tail_d = [], []
    for instance in instances:
        c, f = data[("control", instance)], data[("chosen", instance)]
...
def main():
    ...
    if which == "filter":
        ...
        print("\nfilter rule, on the MEAN: mean(chosen - control) = %+.2f;" % mean)
        ...
        print("\nlook %d of %d, symmetric boundary p <= %.4f (Pocock, six looks)"
              % (last, WAVES, POCOCK))
        print("PRIMARY, per-instance mean: %+.2f, chosen better on %d of %d, "
              "W = %s, p = %.4f"
              % (statistics.mean(mean_d), better, len(mean_d), W, p))
        print("reported, deciding nothing: best-of-%d moved %+.2f"
              % (BLOCK, statistics.mean(tail_d)))
        if p <= POCOCK and lower:
            print("-> ACCEPT: the fix crosses on the lower rank at wave %d" % last)
