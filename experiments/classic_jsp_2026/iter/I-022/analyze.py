#!/usr/bin/env python3
"""I-022: the pre-declared analysis of the record attempt.
...
def collect(cell, instance):
    ...
    values = []
    for tag in tags():
        directory = os.path.join(EXPERIMENT, "results", "I-022_%s_%s" % (tag, cell))
...
def main():
    ...
    print("\n=== 4. best of blocks of %d runs, current against previous ===" % BLOCK)
    wins = losses = ties = 0
    for inst in insts:
        blocks = {}
        for cell in CELLS:
        ...
        pairs = list(zip(blocks["previous"], blocks["current"]))
        w = sum(1 for c, f in pairs if f < c)
        l = sum(1 for c, f in pairs if f > c)
        t = sum(1 for c, f in pairs if f == c)
        wins += w; losses += l; ties += t
        print("%-6s current better on %d of %d blocks (worse %d, tied %d)"
              % (inst, w, len(pairs), l, t))
    print("pooled: current better on %d, worse on %d, tied %d -> sign test p = %.3f"
