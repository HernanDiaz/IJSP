#!/usr/bin/env python3
"""I-034: the pre-declared analysis.
...
0. MECHANISM CHECK, printed first and read first, in two halves, because the
   ...
   tested:
     a) the scouts polished and the tabu calls spent on them: every scout
        in the polish cell, none in the control;
     b) the generations per run, which the pre-launch check saw RISE (92 to
        118, 203 to 288) along with the number of scouts.
...
Filter rule: discard if the mean of (polish - control) over the four filter
...
STATS = ["Scouts polished",
         "Tabu calls on scouts",
...
def directories(tags, cell):
    ...
    for tag in tags:
        out += sorted(glob.glob(os.path.join(
            EXPERIMENT, "results", "I-034_%s_p*_%s" % (tag, cell))))
    return out
...
def report(data, tags, instances, label):
    ...
    print("%-6s %-42s %10s %10s" % ("inst", "statistic", "control", "polish"))
    shares = []
    for instance in instances:
        for key in STATS:
            c = solver_stat(tags, "control", instance, key)
            f = solver_stat(tags, "polish", instance, key)
            if key == STATS[0]:
                shares.append(f)
            print("%-6s %-42s %10.2f %10.2f" % (instance, key, c, f))
    if shares:
        mean_share = statistics.mean(s for s in shares if s == s)
        print("\nscouts polished per run: %.0f on average (the control polishes none)" % mean_share)

    print("\n=== I-034 %s: per-instance mean (primary), best-of-%d (reported) ==="
          % (label, BLOCK))
    print("%-6s %11s %11s %9s   %11s %11s %9s"
          % ("inst", "mean ctrl", "mean polish", "d", "bo5 ctrl", "bo5 polish", "d"))
    mean_d, tail_d = [], []
    for instance in instances:
        c, f = data[("control", instance)], data[("polish", instance)]
...
def main():
    ...
    if which == "filter":
        ...
        print("\nfilter rule, on the MEAN: mean(polish - control) = %+.2f;" % mean)
        ...
        print("\nlook %d of %d, symmetric boundary p <= %.4f (Pocock, six looks)"
              % (last, WAVES, POCOCK))
        print("PRIMARY, per-instance mean: %+.2f, polish better on %d of %d, "
              "W = %s, p = %.4f"
              % (statistics.mean(mean_d), better, len(mean_d), W, p))
        print("reported, deciding nothing: best-of-%d moved %+.2f"
              % (BLOCK, statistics.mean(tail_d)))
        if p <= POCOCK and lower:
            print("-> ACCEPT: polished scouts cross on the lower rank at wave %d" % last)
