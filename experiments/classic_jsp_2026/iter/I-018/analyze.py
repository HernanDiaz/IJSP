#!/usr/bin/env python3
"""I-018: the pre-declared analysis.
...
0. MECHANISM CHECK, printed first and read first, in two halves, because the
   ...
   tested:
     a) where the second tabu call goes: in the best cell every one must
        search the better child again and none the other, while the control
        splits them about 53 / 47 (measured 2026-09-23);
     b) the generations per run: re-searching a local optimum is cheap, so
        they should rise (191 to 237 on ta45 in the pre-launch check).
...
Filter rule: discard if the mean of (best - control) over the four filter
...
STATS = ["LS second call on the searched best",
         "LS second call on the other child",
...
def directories(tags, cell):
    ...
    for tag in tags:
        out += sorted(glob.glob(os.path.join(
            EXPERIMENT, "results", "I-018_%s_p*_%s" % (tag, cell))))
    return out
...
def report(data, tags, instances, label):
    ...
    print("%-6s %-42s %10s %10s" % ("inst", "statistic", "control", "best"))
    shares = []
    for instance in instances:
        for key in STATS:
            c = solver_stat(tags, "control", instance, key)
            f = solver_stat(tags, "best", instance, key)
            if key == STATS[0]:
                shares.append(f)
            print("%-6s %-42s %10.2f %10.2f" % (instance, key, c, f))
    if shares:
        mean_share = statistics.mean(s for s in shares if s == s)
        print("\nsecond calls on the better child per run: %.0f on average"
              % mean_share)
        print("(the control sends a little over half of them there, by accident)")

    print("\n=== I-018 %s: per-instance mean (primary), best-of-%d (reported) ==="
          % (label, BLOCK))
    print("%-6s %11s %11s %9s   %11s %11s %9s"
          % ("inst", "mean ctrl", "mean best", "d", "bo5 ctrl", "bo5 best", "d"))
    mean_d, tail_d = [], []
    for instance in instances:
        c, f = data[("control", instance)], data[("best", instance)]
...
def main():
    ...
    if which == "filter":
        ...
        print("\nfilter rule, on the MEAN: mean(best - control) = %+.2f;" % mean)
        ...
        print("\nlook %d of %d, symmetric boundary p <= %.4f (Pocock, six looks)"
              % (last, WAVES, POCOCK))
        print("PRIMARY, per-instance mean: %+.2f, best better on %d of %d, "
              "W = %s, p = %.4f"
