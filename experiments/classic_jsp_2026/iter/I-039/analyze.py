#!/usr/bin/env python3
"""I-039: the pre-declared analysis.
...
0. MECHANISM CHECK, printed first and read first, in two halves, because the
   ...
   tested:
     a) the crossings with a drawn mask, the mean keep probability (about
        1/2) and its mean deviation from 1/2 (about 1/4 for a uniform draw):
        every crossing in the umask cell, none in the control;
     b) the generations per run, which the check saw rise.
...
Filter rule: discard if the mean of (umask - control) over the four filter
...
def directories(tags, cell):
    ...
    for tag in tags:
        out += sorted(glob.glob(os.path.join(
            EXPERIMENT, "results", "I-039_%s_p*_%s" % (tag, cell))))
    return out
...
def solver_stat(tags, cell, instance, key):
    ...
    if key.startswith("log:"):
        # The JOX mask counters, printed to stderr at exit, in the job log
        field = key[4:]
        for directory in directories(tags, cell):
            ...
            for line in open(path):
                if line.startswith("JOX crossings "):
                    part = [p.strip() for p in line.split(",")]
                    table = {"JOX crossings": part[0].split()[-1],
                             "mean keep": part[1].split()[-1],
                             "mean deviation": part[2].split()[-1]}
...
def report(data, tags, instances, label):
    ...
    print("%-6s %-42s %10s %10s" % ("inst", "statistic", "control", "umask"))
    shares = []
    for instance in instances:
        for key in STATS:
            c = solver_stat(tags, "control", instance, key)
            f = solver_stat(tags, "umask", instance, key)
            if key == STATS[0]:
                shares.append(f)
            print("%-6s %-42s %10.2f %10.2f" % (instance, key, c, f))
    if shares:
        mean_share = statistics.mean(s for s in shares if s == s)
        print("\nJOX crossings with a drawn mask per run: %.0f on average (the control makes none)" % mean_share)

    print("\n=== I-039 %s: per-instance mean (primary), best-of-%d (reported) ==="
          % (label, BLOCK))
    print("%-6s %11s %11s %9s   %11s %11s %9s"
          % ("inst", "mean ctrl", "mean umsk", "d", "bo5 ctrl", "bo5 umsk", "d"))
    mean_d, tail_d = [], []
    for instance in instances:
        c, f = data[("control", instance)], data[("umask", instance)]
...
def main():
    ...
    if which == "filter":
        ...
        print("\nfilter rule, on the MEAN: mean(umask - control) = %+.2f;" % mean)
        ...
        print("\nlook %d of %d, symmetric boundary p <= %.4f (Pocock, six looks)"
              % (last, WAVES, POCOCK))
        print("PRIMARY, per-instance mean: %+.2f, umask better on %d of %d, "
              "W = %s, p = %.4f"
              % (statistics.mean(mean_d), better, len(mean_d), W, p))
        print("reported, deciding nothing: best-of-%d moved %+.2f"
              % (BLOCK, statistics.mean(tail_d)))
        if p <= POCOCK and lower:
            print("-> ACCEPT: the uniform mask crosses on the lower rank at wave %d" % last)
