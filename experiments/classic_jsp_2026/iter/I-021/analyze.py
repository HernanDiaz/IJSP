#!/usr/bin/env python3
"""I-021: the pre-declared analysis.
...
0. MECHANISM CHECK, printed first and read first, in two halves, because the
   ...
   tested:
     a) the repeat calls beyond the second: substantial in the repeat cell,
        zero in the control, with the longest chain reported;
     b) the generations per run, the price of the extra depth: 6 to 22 %
        fewer in the pre-launch check.
...
Filter rule: discard if the mean of (repeat - control) over the four filter
...
STATS = ["LS repeat calls beyond the second",
         "LS longest chain of calls on one child",
...
def directories(tags, cell):
    ...
    for tag in tags:
        out += sorted(glob.glob(os.path.join(
            EXPERIMENT, "results", "I-021_%s_p*_%s" % (tag, cell))))
    return out
...
def report(data, tags, instances, label):
    ...
    print("%-6s %-42s %10s %10s" % ("inst", "statistic", "control", "repeat"))
    shares = []
    for instance in instances:
        for key in STATS:
            c = solver_stat(tags, "control", instance, key)
            f = solver_stat(tags, "repeat", instance, key)
            if key == STATS[0]:
                shares.append(f)
            print("%-6s %-42s %10.2f %10.2f" % (instance, key, c, f))
    if shares:
        mean_share = statistics.mean(s for s in shares if s == s)
        print("\nrepeat calls beyond the second per run: %.0f on average" % mean_share)
        print("(the control makes none)")

    print("\n=== I-021 %s: per-instance mean (primary), best-of-%d (reported) ==="
          % (label, BLOCK))
    print("%-6s %11s %11s %9s   %11s %11s %9s"
          % ("inst", "mean ctrl", "mean repeat", "d", "bo5 ctrl", "bo5 repeat", "d"))
    mean_d, tail_d = [], []
    for instance in instances:
        c, f = data[("control", instance)], data[("repeat", instance)]
...
def main():
    ...
    if which == "filter":
        ...
        print("\nfilter rule, on the MEAN: mean(repeat - control) = %+.2f;" % mean)
        ...
        print("\nlook %d of %d, symmetric boundary p <= %.4f (Pocock, six looks)"
              % (last, WAVES, POCOCK))
        print("PRIMARY, per-instance mean: %+.2f, repeat better on %d of %d, "
              "W = %s, p = %.4f"
              % (statistics.mean(mean_d), better, len(mean_d), W, p))
        print("reported, deciding nothing: best-of-%d moved %+.2f"
              % (BLOCK, statistics.mean(tail_d)))
        if p <= POCOCK and lower:
            print("-> ACCEPT: the repeat crosses on the lower rank at wave %d" % last)
