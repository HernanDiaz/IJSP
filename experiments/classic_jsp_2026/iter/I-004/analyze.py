#!/usr/bin/env python3
"""I-004: the pre-declared analysis of the filter or of the waves so far.
...
discard. Discard if the mean of (fulltails - control) over the four exceeds +2.0
...
def report(data, instances, label):
    print("=== I-004 %s: per-instance mean (best in brackets) ===" % label)
    print("%-6s" % "inst" + "".join("%18s" % c for c in CELLS) + "%13s" % "ftails - ctrl")
    differences = []
    for instance in instances:
        row = ""
        for cell in CELLS:
        ...
        d = (statistics.mean(data[("fulltails", instance)])
...
def main():
    ...
    if which == "filter":
        ...
        print("\nfilter rule: mean(fulltails - control) = %+.2f; discard if > %+.1f -> %s"
              % (mean, FILTER_RULE,
        ...
        print("\nlook %d of %d, symmetric boundary p <= %.4f (Pocock, six looks)"
              % (last, WAVES, POCOCK))
        print("fulltails vs control: mean %+.2f, fulltails better on %d of %d, W = %s, p = %.4f"
              % (statistics.mean(d), better, len(d), W, p))
        if p <= POCOCK and ft_lower:
            print("-> ACCEPT: fulltails crosses the boundary on the lower rank at wave %d" % last)
