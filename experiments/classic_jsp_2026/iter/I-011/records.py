#!/usr/bin/env python3
"""Any record or match in I-011's 1260 confirmation runs?
...
INSTANCES = sorted({line.split("\t")[2].strip()
                    for line in open(os.path.join(EXPERIMENT, "iter", "I-011",
                                                  "jobs_w1.tsv"))
...
for instance in INSTANCES:
    best = None
    for cell in ("control", "portfolio"):
        for directory in sorted(glob.glob(os.path.join(
                EXPERIMENT, "results", "I-011_w*_p*_%s" % cell))):
