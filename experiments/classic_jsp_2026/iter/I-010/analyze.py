#!/usr/bin/env python3
"""I-010: the pre-declared analysis of the filter or of the waves so far.
...
discard. Discard if the mean of (deepescape - control) over the four exceeds +2.0
...
def report(data, instances, label):
    print("=== I-010 %s: per-instance mean (best in brackets) ===" % label)
    print("%-6s" % "inst" + "".join("%18s" % c for c in CELLS) + "%13s" % "dscp - ctrl")
    differences = []
    for instance in instances:
        row = ""
        for cell in CELLS:
        ...
        d = (statistics.mean(data[("deepescape", instance)])
...
def main():
    ...
    if which == "filter":
        ...
        print("\nfilter rule: mean(deepescape - control) = %+.2f; discard if > %+.1f -> %s"
              % (mean, FILTER_RULE,
        ...
        print("\nlook %d of %d, symmetric boundary p <= %.4f (Pocock, six looks)"
              % (last, WAVES, POCOCK))
        print("deepescape vs control: mean %+.2f, deepescape better on %d of %d, W = %s, p = %.4f"
              % (statistics.mean(d), better, len(d), W, p))
        if p <= POCOCK and ft_lower:
            print("-> ACCEPT: deepescape crosses the boundary on the lower rank at wave %d" % last)
