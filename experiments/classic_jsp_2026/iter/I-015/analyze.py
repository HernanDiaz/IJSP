#!/usr/bin/env python3
"""I-015: the pre-declared analysis.
...
0. MECHANISM CHECK, printed first and read first, in two halves, because the
   ...
   tested:
     a) the plateau moves admitted per run must be substantial, or the veto
        was not deciding anything. The pre-launch check turned 1,304 + 93
        vetoes into 1,898 + 287 admissions on ta29;
     b) the generations per run, which says what keeping those moves costs:
        the check showed a small drop, 95 to 91 on ta29 and 195 to 181 on
        ta41.
...
Filter rule: discard if the mean of (allow - control) over the four filter
...
STATS = ["Plateau moves admitted in local search",
         "Plateau moves admitted in crossover",
...
def directories(tags, cell):
    ...
    for tag in tags:
        out += sorted(glob.glob(os.path.join(
            EXPERIMENT, "results", "I-015_%s_p*_%s" % (tag, cell))))
    return out
...
def report(data, tags, instances, label):
    ...
    print("%-6s %-42s %10s %10s" % ("inst", "statistic", "control", "allow"))
    shares = []
    for instance in instances:
        for key in STATS:
            c = solver_stat(tags, "control", instance, key)
            f = solver_stat(tags, "allow", instance, key)
            if key == STATS[0]:
                shares.append(f)
            print("%-6s %-42s %10.2f %10.2f" % (instance, key, c, f))
    if shares:
        mean_share = statistics.mean(s for s in shares if s == s)
        print("\nplateau moves admitted in the local search per run: %.1f on average"
              % mean_share)
        if mean_share < 50.0:
            print("WARNING: the mechanism hardly fires. Read the endpoint")
            print("knowing the idea was not exercised.")

    print("\n=== I-015 %s: per-instance mean (primary), best-of-%d (reported) ==="
          % (label, BLOCK))
    print("%-6s %11s %11s %9s   %11s %11s %9s"
          % ("inst", "mean ctrl", "mean allow", "d", "bo5 ctrl", "bo5 allow", "d"))
    mean_d, tail_d = [], []
    for instance in instances:
        c, f = data[("control", instance)], data[("allow", instance)]
...
def main():
    ...
    if which == "filter":
        ...
        print("\nfilter rule, on the MEAN: mean(allow - control) = %+.2f;" % mean)
        ...
        print("\nlook %d of %d, symmetric boundary p <= %.4f (Pocock, six looks)"
              % (last, WAVES, POCOCK))
        print("PRIMARY, per-instance mean: %+.2f, allow better on %d of %d, "
              "W = %s, p = %.4f"
              % (statistics.mean(mean_d), better, len(mean_d), W, p))
        print("reported, deciding nothing: best-of-%d moved %+.2f"
              % (BLOCK, statistics.mean(tail_d)))
        if p <= POCOCK and lower:
            print("-> ACCEPT: lifting the veto crosses on the lower rank at wave %d" % last)
