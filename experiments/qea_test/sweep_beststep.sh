#!/bin/bash
# Sweep for best_step_factor on top of target_w 0.0->1.0 baseline.
cd /mnt/c/Users/diazhernan/CLionProjects/IJSP
OUT=experiments/qea_test/sweep_beststep
rm -rf "$OUT"; mkdir -p "$OUT"
RUNS=8; GEN=500
INSTS="ft10_10 ft20_05 la25_04"

FACTORS=(1.0 0.7 0.85 1.15 1.3 1.5 1.7)

for bsf in "${FACTORS[@]}"; do
  for inst in $INSTS; do
    d="$OUT/bsf${bsf}_${inst}"
    mkdir -p "$d"
    cat > "$d/setup.txt" <<EOF
algorithm = QEA
seed = 1
