#!/bin/bash
# Sweep for cosine warm restarts: restart_period on top of the current
# baseline (cosine rot 0.02->0.08 + floor const 0.005 + samples 100->400).
cd /mnt/c/Users/diazhernan/CLionProjects/IJSP
OUT=experiments/qea_test/sweep_restart
rm -rf "$OUT"; mkdir -p "$OUT"
RUNS=8; GEN=500
INSTS="ft10_10 ft20_05 la25_04"

# 0 = no restart (baseline). Others = period in generations.
PERIODS=(0 50 100 125 250)

for rp in "${PERIODS[@]}"; do
