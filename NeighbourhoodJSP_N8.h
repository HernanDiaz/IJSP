/*
 * NeighbourhoodJSP_N8.h
 *
 * N8 neighbourhood for Makespan minimization in JSP.
 *
 * Extends N2 with extra-block reinsertion moves:
 *   - N2 boundary arc swaps (inherited from NB_ParallelN2_MakespanJSP)
 *   - Extra-block reinsertion: move a critical task T to a position outside
 *     its current critical block on the same machine.
 *
 * A move is represented by NeighbourJSP_Arc(x, y, z, tipo):
 ...
 *
 *   Adaptation to JSP (interval processing times, dual G-/G+ extreme graphs):
...
class NB_ParallelN8_MakespanJSP : public NB_ParallelN2_MakespanJSP {

public:
    NB_ParallelN8_MakespanJSP(FuzzyFW::ParameterDB *parameters = NULL)
        : NB_ParallelN2_MakespanJSP(parameters) { }

    NB_ParallelN8_MakespanJSP(const NB_ParallelN8_MakespanJSP & source)
        : NB_ParallelN2_MakespanJSP(source) { }

    virtual Neighbourhood * clone() const {
        return new NB_ParallelN8_MakespanJSP(*this);
    }
